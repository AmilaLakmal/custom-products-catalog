...
import { Box, Text, Image, Breadcrumbs } from "@wix/design-system";
...
type SupportedQueryFields =
  | Parameters<products.ProductsQueryBuilder["ascending"]>[0]
  | Parameters<products.ProductsQueryBuilder["descending"]>[0];

const productTypeToDisplayName: {
  [key in products.ProductType]: string | undefined;
} = {
...
function Products() {
  ...
  const tableState = useTableCollection<products.Product, TableFilters>({
    ...
    filters: {
      ...
      productType: stringsArrayFilter({
        itemName: (p) => productTypeToDisplayName[p] ?? p,
      }),
    },
  });
    ...
    predicate: ({ search, filters }) => {
      return (product) => {
        ...
        if (
          filters.productType &&
          product.productType &&
          filters.productType.indexOf(product.productType) === -1
        ) {
          return false;
        }
          ...
          columns={[
            ...
            },
            {
              ...
              render: (row: products.Product) => {
                ...
                return (
                  productTypeToDisplayName[row.productType] ?? row.productType
                );
              },
              width: "100px",
            },
